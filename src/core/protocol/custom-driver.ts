...
export function buildCustomProviderSystemPrompt(tools: ToolSpec[], ctx: PromptContext): string {
  ...
  return [
    ...
    "## FILE REFERENCES",
    "",
    "When the user's message contains `@[filename](path)`, it means they referenced that file.",
    "Use read_file with the given path to read its contents when needed for the task.",
    "When the message contains a `[Selected in filename]` block, that is an active text selection",
    "from the user's editor — treat it as the primary context for the task.",
    "",
    "## SELF-RECOVERY RULES",
    "",
