...
const en: Catalog = {
  ...
  "cli.cmd.index": "Build or update the repository's semantic index (RAG)",
  "cli.cmd.retrieve": "Retrieve semantically relevant context for a query",
  "cli.arg.retrieveQuery": "natural-language query to search the index for",
  "cli.opt.indexRebuild": "re-embed every file, ignoring the cached index",
  "cli.opt.embProvider": "embeddings provider: ollama | openai-compatible",
  "cli.opt.embModel": "embedding model, e.g. nomic-embed-text or text-embedding-3-small",
  "cli.opt.embBaseUrl": "embeddings base URL (required for openai-compatible)",
  "cli.opt.embApiKey": "embeddings API key (prefer an env ref like ${OPENAI_API_KEY})",
  "cli.opt.topK": "how many chunks to return",
  "retrieval.noBaseUrl": "openai-compatible embeddings need a base URL (--base-url or config.embeddings.baseUrl).",
  "retrieval.notConfigured": "Embeddings are not configured. Run: polypus index --provider <ollama|openai-compatible> --model <model> [--base-url <url>]",
  "retrieval.noIndex": "No repository index found. Run `polypus index` first.",
  "retrieval.configured": "✓ Embeddings configured: {provider} / {model}.",
  "retrieval.indexing": "Indexing {workspace} …",
  "retrieval.indexed": "✓ Index updated: {embedded} embedded, {reused} reused, {skipped} skipped — {chunks} chunks.",
  "retrieval.noResults": "No relevant context found for that query.",
  "retrieval.injectedHeader": "Retrieved repository context",
  ...
  "cli.opt.stream": "with --json: emit NDJSON events live (one per line) for streaming UIs",
  "cli.opt.verify": "force verification on: run project checks (typecheck/build/test/lint) before accepting 'done' and iterate until they pass",
  "cli.opt.noVerify": "force verification off (skip project checks before finishing)",
  "cli.opt.fast": "fast profile: verification, plan-first and auto-context OFF (cheapest, quickest)",
  "cli.opt.quality": "quality profile (default): verification, plan-first and auto-context ON",
  "cli.opt.think": "stream the model's reasoning/chain-of-thought (when the model supports it)",
  "cli.opt.modelOverride": "override the agent's model for this run (OpenRouter model id)",
  "cli.opt.budget": "stop the run when the estimated session cost reaches this USD amount (OpenRouter pricing)",
  "cli.cmd.usage": "Show token/cost analytics by model and per day",
  "cli.opt.usageGlobal": "aggregate usage across all projects (global log) instead of this project",
  ...
  "cli.cmd.rewind": "Fork a session truncated to its first N user turns (non-destructive)",
  "cli.opt.rewindTurns": "number of user turns to keep",
  "cli.cmd.checkpoints": "List a session's file checkpoints, or restore the workspace to one (--restore N)",
  "cli.opt.ckptRestore": "checkpoint number to roll the workspace back to",
  "cli.opt.ckptFile": "restore only this file",
  "cli.opt.ckptDir": "workspace root to restore into (default: current directory)",
  "cli.opt.continue": "resume the most recent saved session",
  "cli.opt.resume": "resume a specific saved session by id",
  "cli.opt.wsPort": "WebSocket server port (default 9876)",
  "cli.opt.wsAllowOrigin": "allowed CORS origin for the WebSocket",
  "cli.cmd.webServer": "Start a WebSocket server that exposes the CLI to extensions/UI via NDJSON stream",
  "webServer.listening": "WebSocket listening on ws://127.0.0.1:{port} · Ctrl+C to stop",
  "webServer.clientConnected": "client connected: {addr}",
  "webServer.running": "running task: {task}",
  ...
  "skills.loaded": "loaded skill(s): {names}",
  "skills.activated": "skill activated: {name} ({scope})",
  "picker.hint": "↑↓ choose · enter select · esc cancel",
  "picker.noMatches": "no matching files",
  "picker.files": "file(s)",
  ...
  "usage.total": "total",
  "usage.runs": "runs",
  "usage.byModel": "Efficiency per model:",
  "usage.scopeProject": "scope: this project (.poly/usage.jsonl)",
  "usage.scopeGlobal": "scope: global (~/.polypus/usage.jsonl)",
  "usage.tryGlobal": "Tip: run `polypus usage --global` to see usage across all projects.",
  "usage.colModel": "model",
  "usage.colRuns": "runs",
  "usage.colTokens": "tokens",
  "usage.colCost": "cost",
  "usage.colPerRun": "cost/run",
  "usage.colPer1k": "US$/1k tok",
  ...
  "checkpoints.empty": "No file checkpoints for this session.",
  "checkpoints.header": "File checkpoints for {id} (oldest first):",
  "checkpoints.hint": "Roll back with `polypus checkpoints <id> --restore <n>`.",
  "checkpoints.restored": "restored {n} file(s) to the state before checkpoint {index}",
  ...
  "repl.verifyShow": "verification is {state} (use /verify on|off)",
  "repl.verifyChanged": "verification → {state}",
  "repl.planFirstShow": "plan-first is {state} (use /planfirst on|off)",
  ...
  "repl.help": [
    ...
    "  /usage [--global]  token/cost analytics by model and per day",
    "  /models [query]  browse OpenRouter models with filters",
    "  /estimate <task>  estimate effort/tokens/cost without running",
    "  /index [--rebuild]  build/update the semantic repo index (RAG)",
    "  /retrieve <query>  query the semantic index (top-K chunks)",
    "  /help            show this help",
    "  /exit            quit",
    "Anything else is sent to the agent as a task. Type / to autocomplete commands.",
  ].join("\n"),
  ...
  "repl.slashList": [
    ...
    "/review\treview mode (confirm each action)",
    "/bypass\tbypass mode (auto-approve)",
    "/quality\tquality profile (verify + plan-first + auto-context)",
    "/fast\tfast profile (scaffolding off, cheapest)",
    "/verify\ttoggle project checks before 'done'",
    "/planfirst\ttoggle a short plan before acting",
    "/swarm <task>\trun a task as a parallel swarm",
    "/usage\ttoken/cost analytics by model and per day",
    "/models\tbrowse OpenRouter models",
    "/estimate <task>\testimate effort/tokens/cost",
    "/index\tbuild/update the semantic index (RAG)",
    "/retrieve <query>\tquery the semantic index",
    "/allow <glob>\tadd a path glob to the allow-list",
    "/reset\tclear the conversation history",
  ...
  "swarm.unverified": "checks failed — not merged",
  ...
  // custom provider
  "customProv.name": "Provider name",
  "customProv.authType": "Authentication type",
  "customProv.authNone": "No authentication — the API is open, just send the request.",
  "customProv.authApiKey": "API Key (Header) — a fixed key is sent in a header on every call.",
  "customProv.authOAuth2": "OAuth2 — Client Credentials (2 calls) — first fetches a token, then uses it in the chat header; the token is refreshed automatically on expiry.",
  "customProv.tokenUrl": "Token endpoint URL",
  "customProv.clientId": "Client ID",
  "customProv.clientSecret": "Client Secret (use ${ENV_VAR})",
  "customProv.tokenPath": "JSONPath to token in auth response",
  "customProv.expiresPath": "JSONPath to expiry seconds (optional)",
  "customProv.headerName": "Header name (e.g. Authorization)",
  "customProv.apiKey": "API Key (use ${ENV_VAR})",
  "customProv.chatUrl": "Chat endpoint URL (may contain {{params.x}})",
  "customProv.bodyTemplate": "Request body template (JSON with {{prompt}})",
  "customProv.responsePath": "JSONPath of reply text (e.g. $.message)",
  "customProv.sessionPath": "JSONPath of session/context ID (optional)",
  "customProv.safetyMode": "Safety mode",
  "customProv.safetyReview": "Review — intercepts file/command suggestions for approval",
  "customProv.safetyReadOnly": "Read-Only — queries only, no local mutations",
  "customProv.safetyBypass": "Bypass — direct pass-through, no guards",
  "customProv.promptRequired": "The body template must contain {{prompt}}.",
  "customProv.testOk": "✅ Connection successful!",
  "customProv.testFail": "❌ Connection failed: {msg}",
  "customProv.authFail": "❌ Auth failed: {msg}",
  "customProv.jsonPathFail": "❌ JSONPath error: {msg}",
  "customProv.saved": "✅ Provider saved successfully!",
  "customProv.inlineSecretWarning": "⚠️ Avoid pasting credentials inline. Prefer ${VAR_NAME} and set the environment variable.",
  "customProv.params": "Detected parameters ({{params.x}})",
};
...
const ptBR: Catalog = {
  ...
  "cli.cmd.index": "Constrói ou atualiza o índice semântico do repositório (RAG)",
  "cli.cmd.retrieve": "Recupera contexto semanticamente relevante para uma consulta",
  "cli.arg.retrieveQuery": "consulta em linguagem natural a buscar no índice",
  "cli.opt.indexRebuild": "re-embeda todos os arquivos, ignorando o índice em cache",
  "cli.opt.embProvider": "provider de embeddings: ollama | openai-compatible",
  "cli.opt.embModel": "modelo de embedding, ex.: nomic-embed-text ou text-embedding-3-small",
  "cli.opt.embBaseUrl": "base URL dos embeddings (obrigatória para openai-compatible)",
  "cli.opt.embApiKey": "API key dos embeddings (prefira um ref de env como ${OPENAI_API_KEY})",
  "cli.opt.topK": "quantos trechos retornar",
  "retrieval.noBaseUrl": "embeddings openai-compatible exigem uma base URL (--base-url ou config.embeddings.baseUrl).",
  "retrieval.notConfigured": "Embeddings não configurados. Rode: polypus index --provider <ollama|openai-compatible> --model <modelo> [--base-url <url>]",
  "retrieval.noIndex": "Nenhum índice do repositório encontrado. Rode `polypus index` primeiro.",
  "retrieval.configured": "✓ Embeddings configurados: {provider} / {model}.",
  "retrieval.indexing": "Indexando {workspace} …",
  "retrieval.indexed": "✓ Índice atualizado: {embedded} embedados, {reused} reusados, {skipped} pulados — {chunks} chunks.",
  "retrieval.noResults": "Nenhum contexto relevante encontrado para essa consulta.",
  "retrieval.injectedHeader": "Contexto recuperado do repositório",
  ...
  "cli.opt.stream": "com --json: emite eventos NDJSON ao vivo (um por linha) para UIs de streaming",
  "cli.opt.verify": "força verificação ligada: roda as checagens do projeto (typecheck/build/test/lint) antes de aceitar 'pronto' e itera até passar",
  "cli.opt.noVerify": "força verificação desligada (pula as checagens antes de finalizar)",
  "cli.opt.fast": "perfil rápido: verificação, plano-primeiro e auto-contexto DESLIGADOS (mais barato e rápido)",
  "cli.opt.quality": "perfil qualidade (padrão): verificação, plano-primeiro e auto-contexto LIGADOS",
  "cli.opt.think": "transmite o raciocínio/chain-of-thought do modelo (quando o modelo suporta)",
  "cli.opt.modelOverride": "sobrescreve o modelo do agente nesta run (id do modelo OpenRouter)",
  "cli.opt.budget": "interrompe a execução quando o custo estimado da sessão atingir este valor em USD (preços do OpenRouter)",
  "cli.cmd.usage": "Mostra analytics de tokens/custo por modelo e por dia",
  "cli.opt.usageGlobal": "agrega o uso de todos os projetos (log global) em vez de só este projeto",
  ...
  "cli.cmd.rewind": "Bifurca uma sessão truncada nos primeiros N turnos do usuário (não-destrutivo)",
  "cli.opt.rewindTurns": "número de turnos do usuário a manter",
  "cli.cmd.checkpoints": "Lista os checkpoints de arquivo de uma sessão, ou restaura o workspace a um (--restore N)",
  "cli.opt.ckptRestore": "número do checkpoint para o qual voltar o workspace",
  "cli.opt.ckptFile": "restaurar apenas este arquivo",
  "cli.opt.ckptDir": "raiz do workspace para restaurar (padrão: diretório atual)",
  "cli.opt.continue": "retoma a sessão salva mais recente",
  "cli.opt.resume": "retoma uma sessão salva específica pelo id",
  "cli.opt.wsPort": "porta do servidor WebSocket (padrão 9876)",
  "cli.opt.wsAllowOrigin": "origem permitida CORS para o WebSocket",
  "cli.cmd.webServer": "Inicia um servidor WebSocket que expõe o CLI para extensões/UI via stream NDJSON",
  "webServer.listening": "WebSocket escutando em ws://127.0.0.1:{port} · Ctrl+C para parar",
  "webServer.clientConnected": "cliente conectado: {addr}",
  "webServer.running": "executando tarefa: {task}",
  ...
  "skills.loaded": "skill(s) carregada(s): {names}",
  "skills.activated": "skill ativada: {name} ({scope})",
  "picker.hint": "↑↓ escolher · enter selecionar · esc cancelar",
  "picker.noMatches": "nenhum arquivo corresponde",
  "picker.files": "arquivo(s)",
  ...
  "usage.byModel": "Eficiência por modelo:",
  "usage.scopeProject": "escopo: este projeto (.poly/usage.jsonl)",
  "usage.scopeGlobal": "escopo: global (~/.polypus/usage.jsonl)",
  "usage.tryGlobal": "Dica: rode `polypus usage --global` para ver o uso de todos os projetos.",
  "usage.colModel": "modelo",
  "usage.colRuns": "execuções",
  "usage.colTokens": "tokens",
  "usage.colCost": "custo",
  "usage.colPerRun": "custo/exec",
  "usage.colPer1k": "US$/1k tok",
  ...
  "checkpoints.empty": "Nenhum checkpoint de arquivo para esta sessão.",
  "checkpoints.header": "Checkpoints de arquivo de {id} (mais antigos primeiro):",
  "checkpoints.hint": "Reverta com `polypus checkpoints <id> --restore <n>`.",
  "checkpoints.restored": "restaurados {n} arquivo(s) ao estado antes do checkpoint {index}",
  ...
  "repl.verifyShow": "verificação está {state} (use /verify on|off)",
  "repl.verifyChanged": "verificação → {state}",
  "repl.planFirstShow": "plano-primeiro está {state} (use /planfirst on|off)",
  ...
  "repl.help": [
    ...
    "  /quality         perfil qualidade: verificação + plano-primeiro + auto-contexto LIGADOS (padrão)",
    "  /fast            perfil rápido: esse reforço DESLIGADO (mais barato e rápido)",
    "  /verify on|off   liga/desliga rodar as checagens do projeto antes de 'pronto'",
    "  /planfirst on|off  liga/desliga forçar um plano curto antes de agir",
    ...
    "  /usage [--global]  analytics de tokens/custo por modelo e por dia",
    "  /models [busca]  navega os modelos do OpenRouter com filtros",
    "  /estimate <task>  estima esforço/tokens/custo sem executar",
    "  /index [--rebuild]  constrói/atualiza o índice semântico (RAG)",
    "  /retrieve <query>  consulta o índice semântico (top-K trechos)",
    "  /help            mostra esta ajuda",
    "  /exit            sair",
    "Qualquer outra coisa é enviada ao agente como tarefa. Digite / para autocompletar comandos.",
  ].join("\n"),
  ...
  "repl.slashList": [
    ...
    "/review\tmodo review (confirma cada ação)",
    "/bypass\tmodo bypass (aprova automaticamente)",
    "/quality\tperfil qualidade (verificação + plano-primeiro + auto-contexto)",
    "/fast\tperfil rápido (reforço desligado, mais barato)",
    "/verify\tliga/desliga checagens antes de 'pronto'",
    "/planfirst\tliga/desliga um plano curto antes de agir",
    "/swarm <task>\troda a tarefa como swarm paralelo",
    "/usage\tanalytics de tokens/custo por modelo e por dia",
    "/models\tnavega os modelos do OpenRouter",
    "/estimate <task>\testima esforço/tokens/custo",
    "/index\tconstrói/atualiza o índice semântico (RAG)",
    "/retrieve <query>\tconsulta o índice semântico",
    "/allow <glob>\tadiciona um glob à allow-list",
    "/reset\tlimpa o histórico da conversa",
    "/sessions\tlista as sessões salvas",
    "/resume <id>\tretoma uma sessão salva",
    "/help\tmostra a ajuda",
    "/exit\tsair",
  ].join("\n"),
  ...
  "swarm.unverified": "checagens falharam — não mesclada",
  ...
  // custom provider
  "customProv.name": "Nome do provedor",
  "customProv.authType": "Tipo de autenticação",
  "customProv.authNone": "Sem autenticação — a API é aberta, basta enviar o request.",
  "customProv.authApiKey": "API Key (Header) — uma chave fixa é enviada em um header a cada chamada.",
  "customProv.authOAuth2": "OAuth2 — Client Credentials (2 chamadas) — primeiro busca um token, depois usa no chat; o token é renovado automaticamente quando expira.",
  "customProv.tokenUrl": "URL do endpoint de token",
  "customProv.clientId": "Client ID",
  "customProv.clientSecret": "Client Secret (use ${ENV_VAR})",
  "customProv.tokenPath": "JSONPath do token na resposta de auth",
  "customProv.expiresPath": "JSONPath de expiração em segundos (opcional)",
  "customProv.headerName": "Nome do header (ex: Authorization)",
  "customProv.apiKey": "API Key (use ${ENV_VAR})",
  "customProv.chatUrl": "URL do endpoint de chat (pode conter {{params.x}})",
  "customProv.bodyTemplate": "Template do body (JSON com {{prompt}})",
  "customProv.responsePath": "JSONPath da resposta de texto (ex: $.message)",
  "customProv.sessionPath": "JSONPath do ID de sessão/contexto (opcional)",
  "customProv.safetyMode": "Modo de segurança",
  "customProv.safetyReview": "Review — intercepta sugestões de arquivo/comando para aprovação",
  "customProv.safetyReadOnly": "Read-Only — apenas consultas, sem mutações locais",
  "customProv.safetyBypass": "Bypass — envio direto sem travas",
  "customProv.promptRequired": "O template do body deve conter {{prompt}}.",
  "customProv.testOk": "✅ Conexão bem-sucedida!",
  "customProv.testFail": "❌ Falha na conexão: {msg}",
  "customProv.authFail": "❌ Falha na autenticação: {msg}",
  "customProv.jsonPathFail": "❌ Erro de JSONPath: {msg}",
  "customProv.saved": "✅ Provedor salvo com sucesso!",
  "customProv.inlineSecretWarning": "⚠️ Evite colar credenciais inline. Prefira ${NOME_VAR} e configure a variável de ambiente.",
  "customProv.params": "Parâmetros detectados ({{params.x}})",
